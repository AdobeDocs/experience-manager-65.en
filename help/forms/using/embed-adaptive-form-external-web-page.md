...
* When the AEM server and the web page are on different domains, [configure absolute request URLs with GuideBridge](#configure-base-url) and perform the steps listed in section, [enable AEM Forms to serve adaptive forms to a cross-domain site](#cross-site).
...
When the embedding web page and AEM publish instance are on different domains, you can use the GuideBridge API to prepend an absolute AEM publish origin to all requests generated by the guideruntime libraries. This is a product-native approach that does not require Apache configuration, CDN workers, load balancer logic, or other external infrastructure components.

The `baseUrl` configuration instructs guideruntime to prepend the specified absolute origin to generated requests, including:

* Form submission requests
* Prefill data retrieval
* Document of Record (DoR) generation
* File uploads
* Internal submit operations


Add the following snippet to the embedding page, alongside the existing `guideBridge.connect` implementation:
...
Replace `https://publish.example.com` with the absolute origin of your AEM publish instance. Do not include a trailing slash.

With this configuration in place, a request that would otherwise resolve as a relative path on the embedding page:

```text
/content/forms/af/my-form/jcr:content/guideContainer.af.submit.jsp
```

is instead sent to the AEM publish instance:

```text
https://publish.example.com/content/forms/af/my-form/jcr:content/guideContainer.af.submit.jsp
```

Because requests are cross-origin when using this approach, configure CORS on the AEM publish instance. See [Configure CORS on AEM publish instance](#configure-cors).
...
### Configure CORS on AEM publish instance {#configure-cors}

When using the GuideBridge `baseUrl` configuration, requests from the embedding page are cross-origin. The AEM publish instance must return appropriate CORS headers for submission, prefill, and Document of Record endpoints. Configure CORS using the standard AEM OSGi configuration:

1. On AEM publish instance, go to AEM Web Console Configuration Manager at `https://'[server]:[port]'/system/console/configMgr`.
1. Locate and open the **Adobe Granite Cross-Origin Resource Sharing Policy** configuration (`com.adobe.granite.cors.impl.CORSPolicyImpl`).
1. Add the origin of the embedding web page to the allowed origins. For example, `https://www.example.com`.
1. Ensure that the allowed paths include the AEM Forms endpoints used by embedded adaptive forms, such as submit, prefill, and Document of Record URLs under `/content/forms/af/`.

Also configure the Apache Sling Referrer Filter so the embedding domain can make POST requests to the AEM server:
