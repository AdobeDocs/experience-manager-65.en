...
uuid: e9a40a80-fd2f-49ff-b92f-3d6d2a687037
contentOwner: Janice Kendall
products: SG_EXPERIENCEMANAGER/6.4/COMMUNITIES
topic-tags: administering
content-type: reference
discoiquuid: 2f8e3e0a-0afc-4cf7-923b-b18be1b8bd93
index: y
internal: n
snippet: y
---
...
For more information regarding user data, visit [Managing Users and User Groups](/6-4/communities/using/users.md).
...
* [User Synchronization](/6-4/sites/administering/using/sync.md)
...
On author, when a community site is published from the [Communities Sites console](/6-4/communities/using/sites-console.md), the effect is to [replicate](/6-4/sites/deploying/using/configuring.md#replication-reverse-replication-and-replication-agents) the associated pages, and Sling distribute the dynamically created community user groups, including their membership.
...
When the topology is a [publish farm](/6-4/communities/using/topologies.md) and user sync has been correctly configured, the *user *and *user profile* is synchronized across the publish farm using Sling distribution.
...
When the [User Administration and Security](/6-4/sites/administering/using/security.md) console is used to add new users in the publish environment, user sync will synchronize the new users and their group membership to other publish instances, if necessary. User sync will also synchronize user groups created through the security console.

### User Posts Content on Publish {#user-posts-content-on-publish}

For user generated content (UGC), the data entered on a publish instance is accessed through the [configured SRP](/6-4/communities/using/srp-config.md).
...
1. If users and user groups have already been created on one publisher, it is recommended to [manually sync](/6-4/sites/administering/using/sync.md#manually-syncing-users-and-user-groups) the user data to all publishers prior to configuring and enabling user sync.  
...
1. Ensure the latest code has been installed:
    ...
    * [AEM platform updates](/kb/aem62-available-hotfixes.md)
    * [AEM Communities updates](/6-4/communities/using/deploy-communities.md#latestfeaturepack)
...
Author instance pings the exporter endpoint of publishers. Whenever a user is created or updated on specific publishers (n), the Author gets the content from their exporter endpoints and [pushes the content](../../../6-5/communities/using/sync.md#main-pars-image-1413756164) to other publishers (n-1, that is apart from the publishers from which the content is fetched).
...
The [authorized user created](/6-4/sites/administering/using/sync.md#createauthuser) on all the publish instances helps the publishers to connect with  author  and configure Sling distribution on the author. This authorized user has all the requisite [ACLs](/6-4/sites/administering/using/sync.md#howtoaddacl).
...
<details> 
 ...
 <ol> 
  <li>Sign in with administrator privileges.</li> 
  <li>Access the <a href="/6-4/sites/deploying/using/configuring-osgi.md">Web Console</a>.<br /> For example, <a href="http://localhost:4502/system/console/configMgr">http://localhost:4502/system/console/configMgr</a>.</li> 
  ...
  <li>Set the username and password to the <a href="/6-4/sites/administering/using/sync.md#createauthorizeduser">authorized user</a>.<br /> For example, <strong> 
...
<details> 
 ...
 <ol> 
  <li>Sign in with administrator privileges.</li> 
  <li>Access the <a href="/6-4/sites/deploying/using/configuring-osgi.md">Web Console.<br /> </a>For example, <a href="http://localhost:4503/system/console/configMgr">http://localhost:4503/system/console/configMgr</a>.</li> 
...
<details> 
 ...
 <p>On each AEM publish instance:</p> 
 <ol> 
  <li>Access the <a href="/6-4/sites/deploying/using/configuring-osgi.md">Web Console</a><br /> for example, <a href="http://localhost:4503/system/console/configMgr">http://localhost:4503/system/console/configMgr</a>.</li> 
...
1. **Check for [improperly added configurations](/6-4/sites/administering/using/sync.md#improperconfig).** Ensure that multiple configurations are not added or edited, instead, the existing default configurations should be edited.
1. **Check configurations**. Ensure that all the [configurations](../../../6-5/communities/using/sync.md#bestpractices)are appropriately set in your AEM Author instance, as mentioned in the [Best Practices](../../../6-5/communities/using/sync.md#main-pars-header-863110628).
1. **Check authorized user permissions**. If the packages are not installed properly, then check that the [authorized user](/6-4/sites/administering/using/sync.md#createauthuser) created in the first Publish instance has the correct ACLs.

   To validate this, instead of the [created authorized user](/6-4/sites/administering/using/sync.md#createauthuser) change the [Adobe Granite Distribution - Encrypted Password Transport Secret Provider](/6-4/sites/administering/using/sync.md#adobegraniteencpasswrd) configuration on Author instance to use Admin user credentials. Now try installing the packages again. If the user sync works fine with administrator credentials, then it means that the created publish user did not have appropriate ACLs.

1. **Check Diff Observer Factory configuration**. If only specific nodes are not synced across the publish farm- for example, group members are not synchronized- then ensure that the [Adobe Granite Distribution - Diff Observer Factory](/6-4/sites/administering/using/sync.md#diffobserver) configuration is enabled and **rep  :members ** are set in **looked properties names**.
...
1. **Check logs generated on user creation on Publish instance**. If the above configurations are appropriately set yet user sync is not working, then check the logs generated on user creation.
   ...
   To debug:
    ...
    1. On AEM author instance, sign in with administrator privileges.  
       2. Access the [Web Console](/6-4/sites/deploying/using/configuring-osgi.md). For example, [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr).  
    ...
    1. Check the [order of logs](../../../6-5/communities/using/sync.md#troubleshoot-sling-distribution-in-aem-communities), created on user creation.  
