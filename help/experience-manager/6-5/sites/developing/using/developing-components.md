...
uuid: 65d98cb1-de08-4972-9f4b-d419a77e75d6
contentOwner: User
products: SG_EXPERIENCEMANAGER/6.5/SITES
topic-tags: components
content-type: reference
discoiquuid: 37618c38-2227-44f6-92ef-43e964c3ede7
...
* When [authoring pages](/6-5/sites/authoring/using/default-components.md), the components allow the authors to edit and configure the content.

    * When constructing a [Commerce](../../../../6-5/sites/administering/using/ecommerce.md) site the components can, for example, collect and render information from the catalog.  
      See [Developing eCommerce](../../../../6-5/sites/developing/using/ecommerce.md) for more information.  
    
    * When constructing a [Communities](../../../../6-5/communities/using/author-communities.md) site the components can provide information to and collect information from your visitors.  
      See [Developing Communities](/6-5/communities/using/communities.md) for more information.

* On the publish instance the components render your content, presenting it as you require to your website visitors.

>[!NOTE]
>
>This page is a continuation of the document [AEM Components - The Basics](/6-5/sites/developing/using/components-basics.md).
...
This page provides the reference documentation (or links to reference documentation) required to develop new components for AEM. See [Developing AEM Components - Code Samples](/6-5/sites/developing/using/developing-components-samples.md) for some practical examples.

## Structure {#structure}

The basic structure of a component is covered on the page [AEM Components - The Basics](/6-5/sites/developing/using/components-basics.md#structure). That document covers both the touch-enabled and classic UIs. Even if you do not need to use the classic settings in your new component it can help to be aware of them when inheriting from existing components.
...
Extending an existing component can be achieved with [Resource Type Hierarchy](/6-5/sites/developing/using/components-basics.md#component-hierarchy-and-inheritance) and the related inheritance mechanisms.

>[!NOTE]
>
>Components can also be redefined with an overlay based on the search path logic. However in such case, the [Sling Resource Merger](/6-5/sites/developing/using/sling-resource-merger.md) will not be triggered and `/apps` must define the entire overlay.

>[!NOTE]
>
>The [content fragment component](../../../../6-5/sites/developing/using/customizing-content-fragments.md) can also be customized and extended, though the full structure and relationships with Assets must be considered.

### Customizing a Existing Component Dialog {#customizing-a-existing-component-dialog}

It is also possible to override a *component dialog* using the [Sling Resource Merger](/6-5/sites/developing/using/sling-resource-merger.md) and defining the property `sling:resourceSuperType`.
...
See the [Sling Resource Merger](/6-5/sites/developing/using/sling-resource-merger.md) for more details.
...
[The HTL Java Use-API enables a HTL file to access helper methods in a custom Java class](/htl/using/use-api-java.md). This allows you to use Java code to implement the logic for selecting and configuring the component content.

### Using JavaScript {#using-javascript}

[The HTL JavaScript Use-API enables a HTL file to access helper code written in JavaScript](/htl/using/use-api-javascript.md). This allows you to use JavaScript code to implement the logic for selecting and configuring the component content.
...
Read [Using Client-Side HTML Libraries](../../../../6-5/sites/developing/using/clientlibs.md) for more information.
...
The [edit behavior of a component is configured](/6-5/sites/developing/using/components-basics.md#edit-behavior) by adding a `cq:editConfig` node of type `cq:EditConfig` below the component node (of type `cq:Component`) and by adding specific properties and child nodes.

## Configuring the Preview Behavior {#configuring-the-preview-behavior}

The [WCM Mode](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.md) cookie is set when switching to **Preview** mode even when the page is not refreshed.
...
>[!NOTE]
>
>In the touch-enabeld UI only the values `EDIT` and `PREVIEW` are used for the [WCM Mode](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.md) cookie.
...
[Coral UI](/6-5/sites/developing/using/reference-materials/coral-ui/coralui3/index.md) and [Granite UI](/6-5/sites/developing/using/reference-materials/granite-ui/api/index.md) define the modern look and feel of AEM.

[Granite UI provides a large range of the basic components (widgets)](/6-5/sites/developing/using/reference-materials/granite-ui/api/index.md) needed to create your dialog on the authoring environment. When necessary you can extend this selection and [create your own widget](#creatinganewwidget).
...
* Coral UI
    ...
    * [Concepts of the AEM Touch-Enabled UI - Coral UI](../../../../6-5/sites/developing/using/touch-ui-concepts.md#coral-ui)
...
* Granite UI
    ...
    * [Concepts of the AEM Touch-Enabled UI - Granite UI](../../../../6-5/sites/developing/using/touch-ui-concepts.md#coral-ui) 
    * [Granite UI Documentation](/6-5/sites/developing/using/reference-materials/granite-ui/api/index.md)

>[!NOTE]
>
>Due to the nature of the Granite UI components (and differences to the ExtJS widgets), there are some differences between how components interact with the touch-enabled UI and the [classic UI](/6-5/sites/developing/using/developing-components-classic.md).
...
>* the related sample code covered under [Code Sample - How to Customize Dialog Fields](/6-5/sites/developing/using/developing-components-samples.md#code-sample-how-to-customize-dialog-fields).  
>
...
To create a new widget for use in a component dialog for the touch-enabled UI requires you to [create a new Granite UI field component](/6-5/sites/developing/using/granite-ui-component.md).

>[!NOTE]
>
>For full details about the Granite UI, please see the [Granite UI documentation](/6-5/sites/developing/using/reference-materials/granite-ui/api/index.md).
...
More specifically Granite UI provides a range of field components that are suitable for use in dialogs (or, more generally speaking, in [forms](/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/form/index.md)).
...
If you want to define styling and behavior for your component, you can create a dedicated [client library](../../../../6-5/sites/developing/using/clientlibs.md) that defines your custom CSS/LESS and JS.
...
* `cqgems/customizingfield/components/colorpicker/clientlibs`
    ...
    * provided by the [Code Sample](/6-5/sites/developing/using/developing-components-samples.md#code-sample-how-to-customize-dialog-fields)
...
The Design dialog is provided when a component has design details that can be edited in [Design Mode](/6-5/sites/authoring/using/default-components-designmode.md).
...
See [Extending Page Authoring - Add New Inplace Editor](/6-5/sites/developing/using/customizing-page-authoring-touch.md#add-new-in-place-editor) for more information.

## Customizing the Component Toolbar {#customizing-the-component-toolbar}

The [Component Toolbar](/6-5/sites/developing/using/touch-ui-structure.md#component-toolbar) gives the user access to a range of actions for the component such as edit, configure, copy, and delete.

See [Extending Page Authoring - Add New Action to a Component Toolbar](/6-5/sites/developing/using/customizing-page-authoring-touch.md#add-new-action-to-a-component-toolbar) for more information.
...
If your new component references content from other pages then you can consider whether you want it to impact the **Borrowed Content** and **Lent Content** sections of the [**References**](../../../../6-5/sites/authoring/using/basic-handling.md#references) Rail.
...
>[!NOTE]
>
>When working with AEM there are several methods of managing the configuration settings for such services. See [Configuring OSGi](/6-5/sites/deploying/using/configuring-osgi.md) for more details and the recommended practices.
...
* using [Design mode](/6-5/sites/authoring/using/default-components-designmode.md) when editing a specific page.
* [defining the `components` property on the paragraph system of a template](/6-5/sites/developing/using/components-basics.md#adding-your-component-to-the-paragraph-system).
...
The [AEM Brackets Extension](/6-5/sites/developing/using/aem-brackets.md) provides a smooth workflow to edit AEM components and client libraries. It is based on the [Brackets](http://brackets.io/) code editor.
...
* Provides some [HTL](/htl/user-guide.md) support, the template language designed to simplify component development and increase security.
...
* HTL
    ...
    * Use of [HTL](/htl/user-guide.md) is not compulsory, but if your component needs updating then it is an ideal time to consider [migrating from JSP to HTL](/6-5/sites/developing/using/components-basics.md#htl-vs-jsp).

* Components

    * Migrate [ `cq:listener`](../../../../6-5/sites/developing/using/developing-components.md#migrating-cq-listener-code) code that use classic UI specific functions
    * RTE plugin, for further information see [Configuring the Rich Text Editor](../../../../6-5/sites/administering/using/rich-text-editor.md).
...
* Dialogs
    ...
    * The [Dialog Conversion Tool](/6-5/sites/developing/using/dialog-conversion.md) is provided to help you extend existing components.
    * [Mapping ExtJS to Granite UI Components](../../../../6-5/sites/developing/using/touch-ui-concepts.md#extjs-and-corresponding-granite-ui-components) provides a convenient overview of ExtJS xtypes and node types with their equivalent Granite UI resource types.
    ...
    * Migrate from vtypes to [Granite UI validation](/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/clientlibs/foundation/js/validation/index.md)
...
All you need to do is place a `README.md` file in the component structure. This markdown will then be displayed in the [component console](../../../../6-5/sites/authoring/using/default-components-console.md).

![](assets/chlimage_1-13.png)

The supported markdown is the same as that for [content fragments](../../../../6-5/assets/using/content-fragments-markdown.md).
