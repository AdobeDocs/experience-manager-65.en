...
uuid: b9c2e77d-ec67-482d-bd81-b28bda487164
contentOwner: msm-service
products: SG_EXPERIENCEMANAGER/6.5/SITES
topic-tags: personalization
content-type: reference
discoiquuid: b452e44b-cdaa-4ad2-8dc3-48a11e095cfe
index: y
internal: n
snippet: y
---
...
>[!NOTE]
>
>Client Context has been superseded by ContextHub in the touch UI. Please see the [related documentation](/6-5/sites/developing/using/contexthub.md) for details.
...
* A [javascript API](/6-5/sites/developing/using/ccjsapi.md) for interacting with session stores.
...
For information about opening Client Context, configuring the information that it displays, and simulating the user experience, see [Client Context](../../../../6-5/sites/administering/using/client-context.md).
...
* The server (see [JSONP Store](../../../../6-5/sites/administering/using/client-context.md#main-pars-variable-8) for storing information from 3rd-party sources)

The Client Context framework provides a [javascript API](/6-5/sites/developing/using/ccjsapi.md) that you can use to interact with session stores to read and write user data, and listen and react to store events. You can also create session stores for user data that you use for content targeting or other purposes.
...
For a description of the installed Context Store Components that you can add to Context Store, see [Available Client Context Components](../../../../6-5/sites/administering/using/client-context.md#available-client-context-components).
...
Profile information is also used for delivering [targeted content](../../../../6-5/sites/authoring/using/content-targeting-touch.md).
...
* ` [CQ_Analytics.SessionStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-sessionstore)`: These objects reside only in the page DOM. The data is created and persisted during the lifetime of the page.
* ` [CQ_Analytics.PerstistedSessionStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-persistedsessionstore)`: These objects reside in the page DOM and are persisted either in browser storage or cookies. The data is available across pages and across user sessions.

The API also provides extensions of these classes that are specialized for storing JSON data or JSONP data:

* Session-only objects: [CQ_Analytics.JSONStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-jsonstore) and [CQ_Analytics.JSONPStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-jsonpstore).

* Persisted objects: [CQ_Analytics.PersistedJSONStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-persistedjsonstore) and [CQ_Analytics.PersistedJSONPStore](/6-5/sites/developing/using/ccjsapi.md#cq-analyics-persistedjsonpstore).

#### Creating the Session Store Object {#creating-the-session-store-object}

The javascript of your client library folder creates and initializes the session store. The session store must then be registered using Context Store Manager. The following example creates and registers a [CQ_Analytics.SessionStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-sessionstore) object.
...
For storing JSON data, the following example creates and registers a [CQ_Analytics.JSONStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-sessionstore) object.
...
Obtain a session store object to read or write data to the store. [CQ_Analytics.ClientContextMgr](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-clientcontextmgr) provides access to stores based on the store name. Once obtained, use the methods of the [CQ_Analytics.SessionStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-sessionstore) or [CQ_Analytics.PersistedSessionStore](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-persistedsessionstore) to interact with store data.
...
The session stores are built on the `Observable` pattern. They extend [ `CQ_Analytics.Observable`](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-observable) that provides the ` [addListener](/6-5/sites/developing/using/ccjsapi.md#addlistener-event-fct-scope)` method.
...
Use the [CQ_Analytics.ClientContextUtils](/6-5/sites/developing/using/ccjsapi.md#cq-analytics-clientcontextutils) object's [onStoreRegistered](/6-5/sites/developing/using/ccjsapi.md#onstoreregistered-storename-callback) and [onStoreInitialized](/6-5/sites/developing/using/ccjsapi.md#onstoreinitialized-storename-callback-delay) methods to access session stores only when they are available. These methods enable you to register event listeners that react to session registration and initialization events.
...
See ` [CQ_Analytics.PersistedSessionStore.setNonPersisted(propertyName)](/6-5/sites/developing/using/ccjsapi.md#setnonpersisted-name)`
