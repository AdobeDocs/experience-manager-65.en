...
uuid: 506816da-90ee-4077-b942-983799ea6650
contentOwner: User
products: SG_EXPERIENCEMANAGER/6.5/SITES
topic-tags: extending-aem
content-type: reference
discoiquuid: fd9599a4-82e0-4da4-adb9-e753e73fd4aa
index: y
internal: n
snippet: y
---
...
>[!NOTE]
>
>This page should be read in conjunction with [Reusing Content: Multi Site Manager](/6-5/sites/administering/using/msm.md).
...
* [com.day.cq.wcm.msm.api](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/package-frame.md)
* [com.day.cq.wcm.msm.commons](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/commons/package-frame.md)

The main MSM API objects interact as follows (see also [Terms Used](/6-5/sites/administering/using/msm.md#terms-used)):

![](assets/chlimage_1-35.png)

* **`Blueprint`** 
  A `Blueprint` (as in [blueprint configuration](/6-5/sites/administering/using/msm.md#source-blueprints-and-blueprint-configurations)) specifies the pages from which a live copy can inherit content. 
...
Create custom synchronization actions to use with your rollout configurations. Create a synchronization action when the [installed actions](../../../../6-5/sites/administering/using/msm-sync.md#installed-synchronization-actions) do not meet your specific application requirements. To do so, create two classes:

* An implementation of the [ `com.day.cq.wcm.msm.api.LiveAction`](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveAction.md) interface that performs the action. 
* An OSGI component that implements the [ `com.day.cq.wcm.msm.api.LiveActionFactory`](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.md) interface and creates instances of your `LiveAction` class.
...
<p>Create custom synchronization actions to use with your rollout configurations. Create a synchronization action when the <a href="../../../../6-5/sites/administering/using/msm-sync.md#installed-synchronization-actions">installed actions</a> do not meet your specific application requirements. To do so, create two classes:</p>
<ul>
<li>An implementation of the <a href="/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveAction.md"><span class="code">com.day.cq.wcm.msm.api.LiveAction</span></a> interface that performs the action. </li>
<li>An OSGI component that implements the <a href="/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.md"><span class="code">com.day.cq.wcm.msm.api.LiveActionFactory</span></a> interface and creates instances of your <span class="code">LiveAction</span> class.</li>
</ul>
...
The parameter of the ` [LiveActionFactory](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.md).createAction` method is a `Resource` object. This `Resource` object represents the `cq:LiveSyncAction` node for this live action in the rollout configuration; see [Creating a Rollout Configuration](../../../../6-5/sites/administering/using/msm-sync.md#creating-a-rollout-configuration). As usual when using a configuration node, you should adapt it to a `ValueMap` object:
...
* A [ `Resource`](/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/Resource.md) object that represents the source of the Live Copy.
* A `Resource` object that represents the target of the Live Copy.
* The [ `LiveRelationship`](/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveRelationship.md) object for the live copy.
...
>[!NOTE]
>
>The `Resource` arguments may be `null` or `Resources` objects that do not adapt to `Node` objects, such as [ `NonExistingResource`](/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/NonExistingResource.md) objects.
...
>[!NOTE]
>
>See also the [best practices for customizing rollouts](/6-5/sites/administering/using/msm-best-practices.md#customizing-rollouts).
...
1. Click **Edit**.
1. In the **Rollout Config** dialog, select the ** [Sync Trigger](../../../../6-5/sites/administering/using/msm-sync.md#rollout-triggers)** to define the action that causes the rollout to occur.
...
1. Click **Create** then **Create Node**. Then configure the following node properties and click **OK**:
    ...
    * **Name**: The node name of the synchronization action. The name must be the same as the **Action Name** in the table under [Synchronization Actions](../../../../6-5/sites/administering/using/msm-sync.md#installed-synchronization-actions), for example `contentCopy` or `workflow`.
    
    * **Type**: `cq:LiveSyncAction`

1. Select the action node just created and add the following property to the node:

    * **Name**: The property name of the action. The name must be the same as the **Property Name** in the table under [Synchronization Actions](../../../../6-5/sites/administering/using/msm-sync.md#installed-synchronization-actions), for example `enabled`.  
    
    * **Type**: String  
    
    * **Value**: the property value of the action. For valid values, see the **Properties** column in [Synchronization Actions](../../../../6-5/sites/administering/using/msm-sync.md#installed-synchronization-actions), for example `true`.
...
* For information about the adobe-public profile, see [Obtaining the Content Package Maven Plugin](/6-5/sites/developing/using/vlt-mavenplugin.md#obtaining-the-content-package-maven-plugin)
...
1. Start Eclipse and [import the Maven project](/6-5/sites/developing/using/howto-projects-eclipse.md#import-the-maven-project-into-eclipse).
...
1. Create and configuration a [Rollout Configuration with the standard procedure](../../../../6-5/sites/administering/using/msm-sync.md#creating-a-rollout-configuration) - and using the properties:
...
[Create a live copy](/6-5/sites/administering/using/msm-livecopy.md#creating-a-live-copy-of-a-page) of the English/Products branch of the We.Retail Reference Site using your rollout configuration:
...
* Contact Email:
    ...
    * Is excluded from the rolled out properties; see [Excluding Properties and Node Types from Synchronization](../../../../6-5/sites/administering/using/msm-sync.md#excluding-properties-and-node-types-from-synchronization).
