...
uuid: 5cec6b1c-3264-4853-b50a-c9fdfc92afe1
contentOwner: aheimoz
products: SG_EXPERIENCEMANAGER/6.5/FORMS
topic-tags: publish
discoiquuid: 520863cb-4f75-4c3a-bc80-b48f0a344a1a
index: y
internal: n
snippet: y
---
...
* **Submitted information: **The following fields listed below serve as output locations to the task:** **
    ...
    * **Save layout template using: **Save the layout template using a path that is relative to the payload or store it in a variable of Document data type. The [layout template](../../../6-5/forms/using/layout-design-details.md) refers to an XDP file that you create using Forms Designer. This option is available only if you select Interactive Communication Agent UI from the Type drop-down list.
...
* **Participant Chooser:** The option is available when the **Dynamically to a user or group** option is selected in the Assign options field. You can use an ECMAScript or a service to dynamically select a user or a group. For more information, see [Dynamically assign a workflow to the users](/kb/HowToAssignAWorkflowDynamicallyToParticipants.md) and [Creating a custom Adobe Experience Manager Dynamic Participant step.](/using/dynamic-steps.md)
...
* **Notify Assignee by Email:** Select this option to send email notifications to the assignee. These notifications are sent when a task is assigned to a user. Before using the option, enable the notifications from AEM Web Console. For step-by-step instructions, see [configure email notifications for the assign task step](../../../6-5/forms/using/aem-forms-workflow.md)
...
The email step uses Day CQ Mail Service to send emails. Before using the email step, ensure that the [email service](../../../6-5/forms/using/aem-forms-workflow.md) is configured. The email step has the following properties:
...
* **Adobe Sign Cloud Configuration**: Choose an Adobe Sign Cloud Configuration. If you have not configured Adobe Sign for AEM Forms, see [Integrate Adobe Sign with AEM Forms](../../../6-5/forms/using/adobe-sign-integration-adaptive-forms.md). 
...
Similar to other AEM Forms workflow steps, such as Assign Task, Send Email, and Sign Document, you can use variables in all AEM Document services steps. For more information on creating and managing variables, see [Variables in AEM workflows](../../../6-5/forms/using/variable-in-aem-workflows.md).
