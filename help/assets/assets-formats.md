...
|---|---|
|   |Storage |Metadata management |Metadata extraction |Thumbnail generation |Interactive editing |Metadata write-back |Insights |Upload (Input format) |Create image preset (Output format) |Preview dynamic rendition |Deliver dynamic rendition |Download dynamic rendition |
| Format |  |
...
>
>* Metadata write-back works for PSB format when it is added to the NComm handler.
...
>* For EPS files, metadata write-back is supported only if the version of PostScript Document Structuring Convention (PS-Adobe) is greater than 3.0.
>
...
**&#42;** The ZIP archives created using the `Deflate64` algorithm have limited support in AEM. Archive and un-archive operations are not supported. However, operations like uploading, browsing, and downloading are supported.
