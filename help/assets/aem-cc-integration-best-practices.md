...
| Use - open asset  | Yes | Yes - for any app  | [Open from Web UI](managing-assets-touch-ui.md#previewing-assets) or from Finder |
...
| Edit - open for editing | Yes - Check-out action | Yes - Open action (in the network share) | [Check-out in AAL](managing-assets-touch-ui.md#previewing-assets) saves the asset to user's creative cloud storage account (synchronized by Creative Cloud app) by default. |
...
| Upload - single file | Yes - uploads current active document| Yes | [Web UI Upload](managing-assets-touch-ui.md#uploading-assets) |
| Upload - multiple files / hierarchical folder structures | No | Yes | [Web UI Upload](managing-assets-touch-ui.md#uploading-assets)<br>Custom script/ tool |
