...
Last Modified Date: 2019-09-17T08:42:06.676-0400
<h3 style="text-align: center;"><strong>AG notes from engg demo, 9/17</strong>:</h3>
<ul>
<li>List of MIME types supported -- get list from filetypes UI in CF.</li>
<li>CFs are not supported.</li>
<li>No change in roles and permissions. It remains the same.</li>
<li>There are no new prerequisites.</li>
<li>Find Async Job UI help content and link to it.</li>
</ul>
<div>
</div>
<div>
<strong>Usage </strong>guidelines:
</div>
<ul>
<li>When publishing the page, AEM lists all the assets used in the page. Validate if remote assets are fetched or not. If not, the page is still published with empty components.<br /> </li>
<li>A quick way to validate is to look in AEM notifications for errors in fetching remote assets.</li>
</ul>
<div>
<strong>Troubleshooting </strong>info:
</div>
<ul>
<li>An asset fetched from remote dam is not published on a web page for the following reasons: it doesn't exist on remote; no permissions to fetch it; network failure. Ensure that appropriate requirements are met and retry adding the asset to the page and republish. Check Async Job UI for errors in fetching.</li>
</ul>
<p><strong>Ask PM</strong>:<br /> </p>
<ul>
<li>Subassets support for fetching when dragged and dropped</li>
<li>Functionality with shared datastore</li>
<li>Script to auto-configure<br /> </li>
<li>Design issues w.r.t<br /> </li>
<li>Not doing 6.4 backport</li>
</ul>
<div>
Report
...
<ul>
<li>Order of filetype predicates in CF must be updated.</li>
<li>Thumbnail is missing when Icon </li>
</ul>
-->
...
1. Access the local AEM Sites instance at `http://[local_sites]:4502`. Click **[!UICONTROL Tools]** > **[!UICONTROL Assets]** > **[!UICONTROL Connected Assets Configuration]** and provide the following values:
   ...
   Last Modified Date: 2019-02-20T07:20:30.499-0500
   Change if THIS option is renamed to Connected DAM Configuration after 6.5L22.
   -->
   ...
   Comment Type: annotation
   Last Modified By: asgupta
   Last Modified Date: 2019-02-20T07:21:33.286-0500
   Change the screenshot if UI is renamed to Connected DAM after 6.5L22.
   -->
...
1. As the assets are already processed and the renditions are fetched, disable the workflow launchers. Adjust the launcher configurations on the local (AEM Sites) instance to exclude the **[!UICONTROL connectedassets]** folder, in which the remote assets are fetched.
   ...
   Comment Type: annotation
   Last Modified By: asgupta
   Last Modified Date: 2019-02-20T07:22:12.858-0500
   Change the folder name in multiple places if it is renamed to connecteddam after 6.5L22.
   -->
...
1. Open the Assets tab and click **[!UICONTROL Log in to Connected Assets]**.
   ...
   Comment Type: annotation
   Last Modified By: asgupta
   Last Modified Date: 2019-02-20T07:23:41.246-0500
   Will this change to Connected Assets tab?
   -->
...
<draft-comment>
<related-links>
<a href="https://helpx.adobe.com/experience-manager/kt/platform-repository/using/cors-security-article-understand.html" target="_blank">Understand Cross-Origin Resource Sharing (CORS) with AEM</a>
</related-links>
</draft-comment>
-->
