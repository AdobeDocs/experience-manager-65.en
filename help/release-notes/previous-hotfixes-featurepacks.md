...
* Users can update system-defined variables, such as `afPath`, `afSubmissionTime` and `signers`, when editing an adaptive form. To resolve the issue, these variables are removed from the form submission data on the client side. Users can use the `FormSubmitInfo` object to retrieve the data for these variables from the server (NPR-33654).
