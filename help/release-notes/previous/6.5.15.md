...
To check the proper installation of AEM Forms JEE 6.5.15.0 service pack, navigate to the directory `[AEM_Forms_JEE_DIR]/patch/AEMForms-6.5.0-0057/log` and inspect the `PatchInstallerProcessing[1-9*].log` file for the mentioned entry:
`[AEM_Forms_JEE_DIR]/patch/AEMForms-6.5.0-0057/xml/RUP_BOM.xml not found! Assuming this component isn't in the installation. Skipping Processing`. In case of the correct installation of AEM Forms JEE 6.5.15.0 service pack this entry is not logged. To fix the installation issue of AEM Forms JEE 6.5.15.0 service pack, manually [add the RUP_BOM.xml file](/help/forms/using/linux-jboss-installation-issue-on-sp15.md) to the XML directory. 
