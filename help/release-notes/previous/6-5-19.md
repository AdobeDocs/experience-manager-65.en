...
* **[Accessibility enhancements in AEM Forms Designer](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)**: AEM Forms Designer introduces the following accessibility improvements:

   * AEM Forms Designer now supports accessible Table of Contents through new `<TOC>` and `<TOCI>` tags, allowing authors to assign these roles to subforms via the Accessibility Tab. The feature supports multilevel TOC structures, labeled entries, numbered references, and span substructures, enabling screen readers to properly identify and navigate TOC content.

   * AEM Forms Designer now supports configuring accessibility properties for table cells, including Scope, Row Span, and Column Span, enabling screen readers and assistive devices to correctly interpret table structure in generated PDFs. Authors can set these properties directly from the Cell Properties dialog and verify them in XML Source view. The configured properties are accurately reflected in the tagged PDF output and can be validated using the Acrobat Table Editor.
