...
* **[List Accessibility Support in AEM Forms Designer](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-5/forms/pdf/using-designer.pdf)**: AEM Forms Designer now enables authors to create accessible List structures by assigning List, List Item, and List Label roles to subforms via the Accessibility Tab. The feature supports simple, numbered, bulleted, and nested lists, with proper `<L>`, `<LI>`, `<Lbl>`, and `<Lbody>` tag generation for screen reader compatibility. This enhancement ensures form lists conform to PDF/UA accessibility standards, improving navigation and content comprehension for assistive technology users.
