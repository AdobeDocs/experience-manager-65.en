...
Using push notifications with AEM requires a few different technologies. First, a push notification service provider must be used to managethenotifications and devices (AEM does not do this, yet). Two providers are configured out-of-the-box with AEM: [Amazon Simple Notification Service](https://aws.amazon.com/sns/) (or SNS), and [Pushwoosh](https://www.pushwoosh.com/). Second, the push technology for the given mobile OS must go through the appropriate service — Apple's Push Notification Service (or APNS) for iOS devices; and Google Cloud Messaging (or GCM) for Android&trade; devices. Although AEM does not communicate with these platform-specific services directly, some related configuration information must be provided by AEM along with the notifications for these services to execute the push.
...
1. The core provider pushes the notification to all devices registered for that push. For each device, it uses the cellular data network or WiFi, whichever is available on the device.
1. The notification displays on the device if the app that it is registered for, is not running. A user tapping on the notification starts the app and display the notification within the app. In the case that the application is already running, only the in-app notification is displayed.

This release of AEM supports iOS and Android&trade; mobile devices.
...
[Follow these steps](https://developer.android.com/google/gcm/gs.html) to **Create a Google API project**, **Enable the GCM Service**, and **Obtain an API Key**. You need the **API Key** to send push notifications to Android&trade; devices. Also, record your **Project Number**, which is also sometimes called a **GCM Sender Id**.
...
1. Choose your app from the list (or create one).
1. Under Android&trade; Package Name, enter your app id, that is, `com.adobe.cq.mobile.weretail.outdoorsapp`. (If that does not work, try again with "test.test".)
...
>[!NOTE]
>
>*Information about Amazon SNS, and a link to create an AWS account, can be found [here](https://aws.amazon.com/sns/). You can get a free account for a year.*
...
1. **Register with Amazon SNS**
    ...
    1. After registering, log in to the management console and select [SNS](https://console.aws.amazon.com/sns/) (Push Notification Service). Click "Get Started" if it appears.
...
1. **Create Access Key and ID**
    ...
    1. Click Access Keys, and in the space below, click **Create New Access Key**.
    1. Click **Show Access Key**, and copy and save the Access Key ID and Secret Access Key shown. If you choose the option to download the keys, you get a csv file that contains those same values.
   ...
   For organizations, which use an "AWS Sandbox" account, the steps are similar, and outlined here:
    ...
    1. Click Users in the left list of actions, and choose your user name.
...
1. **Create Platform Applications**
    ...
    1. Click Applications, then Create Platform Application. Choose a name and select a platform (APNS for iOS, GCM for Android&trade;). Depending on the platform. other fields must be filled in:
    ...
    1. Repeat the above step once for each platform that you are supporting. To be able to push to both iOS and Android&trade;, two Platform Applications must be created.

1. **Create an Identity Pool**

    1. Use [Cognito](https://console.aws.amazon.com/cognito) to create an Identity Pool, which will store basic data of unauthenticated users. Note, only "us-east" and "eu" regions are supported by Amazon Cognito currently.
    ...
    1. On the upper right of the page, click the link "*Edit identity pool"*. The Identity Pool Id is shown. Save this text for later.
...
1. **Configure Access**
    ...
    1. Log in to [Identity and Access Management](https://console.aws.amazon.com/iam/home) (IAM).
    1. Select Roles.
    1. Click the role created in the previous step, called Cognito_&lt;yourIdentityPoolName&gt;Unauth_Role. Record the "Role ARN" displayed.
...
1. **Create an app**
    ...
    1. For Android&trade; support, you must provide your GCM API key.
    1. When configuring the app, choose Cordova as the framework.
    1. For iOS support you must provide the Certificate file (.cer), Push Certificate (.p12) and private key password; these should have been obtained from Apple's APNS site. For Framework, choose Cordova.
...
1. Double-click the embed section in the properties pane.
1. In the dialog box that appears, add a client lib by clicking the + button.
...
For iOS, use a macOS computer and join the [iOS Developer Program](https://developer.apple.com/programs/ios/). Some corporations have corporate licenses which may be available to all developers.
...
#### Android&trade; {#android}

To install the app on an Android&trade; phone using CLI (see below: **Step 6 - Build and deploy the app**), you first must put the phone in "developer mode." See [Enabling On-device Developer Options](https://developer.android.com/tools/device.html#developer-device-options) for details on doing this.
...
1. Log in to AEM as the appropriate user, click the Apps tab.
1. Click the App.
1. Find the Manage Cloud Services tile and click the pencil, to modify your cloud configs.
1. Select Amazon SNS Connection, Pushwoosh Connection, or Adobe Mobile Services, as the notification configuration.
1. Enter the provider properties and click Submit to save them, and Done. They are not verified remotely at this stage, except if there is AMS.
...
**Note:** See the instructions [here](/help/mobile/building-app-mobile-phonegap.md) on building PhoneGap applications.
...
1. Dealing with the results
    ...
    * Note that there are some platform differences between iOS and Android&trade; push notifications. Among them:

        * Building with CLI will start the app after it is deployed on Android&trade;. On iOS, you have to start it manually. Because the push registration step happens at startup, Android&trade; apps can receive push notifications right away (because it has already started and registered), while iOS apps cannot.
        * On Android&trade;, the OK button text is in all caps (and in any other buttons added on the in-app notification), while in iOS it is not.
